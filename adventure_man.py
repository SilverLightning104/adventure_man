...
basic_walk = [load_image('assets/images/characters/platformPack_tile011a.png'),
              load_image('assets/images/characters/platformPack_tile011b.png')]
   
basic_enemy_images = { "walk_lt": basic_walk,
                       "walk_rt": [flip_image(img) for img in basic_walk] }
...
class Hero(pygame.sprite.Sprite):
    ...
    def shoot(self, bullets):
        if self.shot_timer <= 0:
            bullet = Bullet(item_images['Bullet'])
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self, level):
        ...
        enemy_list = pygame.sprite.spritecollide(self, level.enemies, False)
        tile_list = pygame.sprite.spritecollide(self, level.main_tiles, False)

        for enemy in enemy_list:
            enemy.health -= 1
        
        if len(enemy_list) > 0 or len(tile_list) > 0:
            self.kill()
    ...
    def process_input(self):     
        ...
        if self.stage == Game.PLAYING:
            ...
            if pressed[pygame.K_SPACE]:
                self.hero.shoot(self.level.bullets)
